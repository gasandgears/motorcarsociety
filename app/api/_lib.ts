...
export async function getOrCreateAccount(request: Request): Promise<Account | null> { const user = getAuthenticatedUser(request); if (!user) return null; const email = user.email.trim().toLowerCase(); const now = Date.now(); const protectedAccount = email === "deank@kirklanddigital.com" || email === "bbforcars@gmail.com" || email === "deankirkland@me.com"; const bootstrap = email === "deank@kirklanddigital.com" || email === "bbforcars@gmail.com" ? { role: "admin", tier: "leadership", status: "approved" } : email === "deankirkland@me.com" ? { role: "member", tier: "standard", status: "approved" } : { role: "member", tier: "free", status: "approved" }; const client = db(); await client.from("accounts").upsert({ user_id: user.id, email, display_name: email.split("@")[0], phone: "", location: "", collection_notes: "", ...bootstrap, created_at: now, updated_at: now }, { onConflict: "user_id", ignoreDuplicates: true }); if (protectedAccount) await client.from("accounts").update({ ...bootstrap, email, updated_at: now }).eq("user_id", user.id); else await client.from("accounts").update({ role: "member", tier: "free", status: "approved", email, updated_at: now }).eq("user_id", user.id).eq("role", "applicant").eq("tier", "none").eq("status", "pending"); const { data } = await client.from("accounts").select("*").eq("user_id", user.id).single(); return data ? mapAccount(data) : null; }
